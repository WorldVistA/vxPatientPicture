...
function GetFutureFMDate(Offset: string): string;
...
function GetSpecificFileFieldData(FileNum : string; IEN : string; FieldNum : string; Flag: string='E'): string;
function TempStoreOfTIUNoteIEN(TIUNoteIEN : string): boolean;
function ClearStoreOfTIUNoteIEN(): boolean;
function GetMUMPSVariableData(VarName : string): string;
...
procedure ListPtByAltLookups(Dest: TStrings; const Lookup: string; From: string; Index: string); //lwm
...
function GetDSSPatientAge(const DFN : string): string; //LWM20060208
function GetDSSSpecPatientDemog(const DFN : string; FieldNum : string): string;  //LWM20060208
function GetDSSSpecPatientMRN(const DFN : string): string; //LWM20061210
function IsInfantPatient: boolean;
function DisplayWeightInLbOzFormat(pWeightString:string):string;
...
//    then Result := Copy(x,1,3) + '-' + Copy(x,4,2) + '-' + Copy(x,6,Length(x))
    then Result := Copy(x,1,3) + '-' + Copy(x,4,3) + '-' + Copy(x,7,Length(x))
    else Result := x;
end;
...
begin
  if Length(Offset) > 0 then
    TestStr := Offset[1];
  if TestStr <> '-' then
    Result := sCallV('VFDC DATE CONVERT', ['T+'+Offset])
  else
    Result := sCallV('VFDC DATE CONVERT', ['T'+Offset]);
end;
...
begin
  ...
  begin
    ...
    {
    slCallParams := TStringList.Create;
    slCallParams.Add('FILE^21650');
    slCallParams.Add('FIELDS^.02');
    slCallParams.Add('INDEX^C');
    slCallParams.Add('VAL^VXCPRS');
    slResults := TStringList.Create;
    tCallV(slResults,'VFDC FM FIND',[slCallParams]);
    if ((slResults.Count = 1) and (Piece(slResults[0],U,1) <> '-1')) then
    begin
     VXCPRS_IEN := Piece(slResults[0],U,1);
    end
    else
    begin
//LWM20061229     ShowMessage('VXCPRS is not correctly configured in file 21650');
     VXCPRS_IEN := '-1';
    end;
    slCallParams.Free;
    slResults.Free;
...
procedure ListPtByAltLookups(Dest: TStrings; const Lookup: string; From: string; Index: string);
var
 cntr : integer;
begin
  ...
  if Lookup <> '' then
  begin
    RPCBrokerV.Param[0].Mult[IntToStr(cntr)] := 'VAL^' + Lookup;
    Inc(cntr);
  end;
  if From <> '' then
  begin
//    RPCBrokerV.Param[0].Mult['"1"'] := 'FROM^' + From;
    RPCBrokerV.Param[0].Mult[IntToStr(cntr)] := 'FROM^' + From;
    Inc(cntr);
  end;
  if Index <> '' then
  begin
//    RPCBrokerV.Param[0].Mult['"2"'] := 'INDEX^' + Index;
    RPCBrokerV.Param[0].Mult[IntToStr(cntr)] := 'INDEX^' + Index;
    Inc(cntr);
  end;
  RPCBrokerV.RemoteProcedure := 'DSIW PATIENT LOOKUP';
  try
    RPCBrokerV.Call;
    if ((RPCBrokerV.Results.Count > 0) and (Piece(RPCBrokerV.Results[0], '^', 1) <> '-1')) then
    begin
...
begin
  ...
  begin
    ...
    //DSSAge := GetDSSPatientAge(DFN);    //LWM
    //MRN := GetDSSSpecPatientMRN(DFN);
  end;
...
begin
  ...
  try
    //UpdateUnsignedOrderAlerts(Patient.DFN);      //moved to AFTER signature and DC actions
    tCallV(tmplst, 'VFDC DDR GETS ENTRY DATA', ['2', DFN, '21601', '']);
    if Piece(tmplst[0],'^',1) <> '-1' then
    begin
     cntr := 0;
     while ((cntr < tmplst.Count-1) and (Piece(tmplst[cntr],'^',3) <> '21601')) do
      Inc(cntr);
     if cntr < tmplst.Count then
      Result := Piece(tmplst[cntr],'^',4);
    end;
  finally
    tmplst.Free;
  end;
...
begin
  ...
  try
    //UpdateUnsignedOrderAlerts(Patient.DFN);      //moved to AFTER signature and DC actions
    tCallV(tmplst, 'VFDC DDR GETS ENTRY DATA', ['2', DFN, FieldNum, '']);
    if Piece(tmplst[0],'^',1) <> '-1' then
    begin
     cntr := 0;
     while ((cntr < tmplst.Count-1) and (Piece(tmplst[cntr],'^',3) <> FieldNum)) do
      Inc(cntr);
     if cntr < tmplst.Count then
      Result := Piece(tmplst[cntr],'^',4);
    end;
...
var
 ...
 tmplst: TStringList;
 MRN : string;
 NumOfInstances : integer;
begin
  Result := '';
  tmplst := TStringList.Create;
  try
    tCallV(tmplst, 'VFDC DDR GETS ENTRY DATA', ['2', DFN, '21600*', '']);
    if (Piece(tmplst[0],'^',1) <> '-1') then
    begin
     NumOfInstances := 0;
     for cntr := 0 to tmplst.Count-1 do
     begin
      if (Piece(tmplst[cntr],U,3) = '.02') then
      begin
       MRN := Piece(tmplst[cntr],U,4);
       Inc(NumOfInstances);
      end;
      if ((Piece(tmplst[cntr],U,3) = '.04') and (Piece(tmplst[cntr],U,4) = 'YES')) then
       Result := MRN;
     end;
     if ((NumOfInstances = 1) and (Result = '')) then
       Result := MRN;
    end;
  finally
    tmplst.Free;
...
function IsInfantPatient: boolean;
begin
 result :=((pos('m', Patient.DSSAge)>0) or (pos('d', Patient.DSSAge)>0) or (pos('w', Patient.DSSAge)>0));
end;

function DisplayWeightInLbOzFormat(pWeightString:string):string;
var
  vWtLBs, vWtOunces: integer;
  vLastWeightLBs: double;
begin
  if IsInfantPatient then
  begin
    vLastWeightLBs := StrToFloat(pWeightString);
    vWtLBs := Trunc(vLastWeightLBs);
    vWtOunces :=  Round(Frac(vLastWeightLBs)*16);
    if vWtOunces=16 then
    begin
      Inc(vWtLBs);
      vWtOunces :=0;
    end;
    result := IntToStr(vWtLBs) +'/'+ IntToStr(vWtOunces);
    //lblLastWeight.Caption := IntToStr(Trunc(vLastWeightLBs))  +'/'+ IntToStr(Round(Frac(vLastWeightLBs)*16));
  end
  else
    result := FloatToStr(vLastWeightLBs);
end;
...
begin
  ...
  tmplst := TStringList.Create;
  try
    tCallV(tmplst, 'VFDC DDR GETS ENTRY DATA', [FileNum, IEN, FieldNum, Flag]);
    if Piece(tmplst[0],'^',1) <> '-1' then
    begin
     cntr := 0;
     while ((cntr < tmplst.Count-1) and (Piece(tmplst[cntr],'^',3) <> FieldNum)) do
...
begin
 Result := false;
 try
  JobNumber := GetMUMPSVariableData('$J');
  slCallParams := TStringList.Create;
{  slCallParams.Add('FILE^21650');
  slCallParams.Add('FIELDS^.02');
  slCallParams.Add('INDEX^C');
  ...
  vxCPRSien := User.vxCPRS_IEN;
  if vxCPRSien <> '-1' then
  begin
   VisitIEN := GetSpecificFileFieldData('8925',TIUNoteIEN,'.03','I');
   slCallParams.Add('PATIENT~'+Patient.DFN);
   slCallParams.Add('KEY1~'+JobNumber);
   slCallParams.Add('KEY2~TIU NOTE IEN');
   slCallParams.Add('APPLICATION~'+vxCPRSien);
   slCallParams.Add('DATA1~'+TIUNoteIEN);
   slCallParams.Add('DATA2~'+VisitIEN);
   if (Piece(sCallV('VFDC ADD INTERPROCESS COM',[slCallParams]),U,1) <> '-1') then
    Result := true;
  end
  else
   ShowMessage('VXCPRS is not correctly configured in file 21650');
 finally
  slCallParams.Free;
 end;
end;
...
begin
 ...
 try
  ...
  begin
   ...
   slCallParams.Add('KEY1~'+JobNumber);
   slCallParams.Add('KEY2~TIU NOTE IEN');
   slCallParams.Add('APPLICATION~'+Piece(slResults[0],U,1));
   if (Piece(sCallV('VFDC DEL INTERPROCESS COM',[slCallParams]),U,1) <> '-1') then
    Result := true;
  end;
 finally
  slCallParams.Free;
...
begin
 RPCBrokerV.ClearParameters := true;
 RPCBrokerV.Param[0].PType := reference;
 RPCBrokerV.Param[0].Value := VarName;
 RPCBrokerV.RemoteProcedure := 'XWB GET VARIABLE VALUE';
 RPCBrokerV.Call;
