...
uses SysUtils, Windows, Classes, Forms, ORFn, uConst, ORClasses, ORNet, Trpcb,
     vxrpcBroker;
...
type
  ...
  public
    ...
    property vxCPRS_IEN: string read FvxCPRS_IEN;
  end;
  ...
  private
    ...
    FMRN  : string;                               // DSS Default Alternate ID
...
//  FCurrentPrinter  := GetDefaultPrinter(DUZ, 0);
  ...
  FvxCPRS_IEN := UserInfo.VXCPRS_IEN;
end;
...
//lwm20061118      UnlockDocument(DocIEN);
//lwm20061118      ConsultIEN := GetConsultIENforNote(DocIEN);
      if ConsultIEN > -1 then
        begin
//lwm20061118          OrderID := GetConsultOrderIEN(ConsultIEN);
//lwm20061118          UnlockOrderIfAble(OrderID);
        end;
...
//lwm20061118  if ItemType = CH_ORD then UnlockOrder(AnID);
//lwm20061118  if ItemType = CH_DOC then UnlockDocument(StrToIntDef(AnID, 0));
//lwm20061118  if ItemType = CH_CON then UnlockDocument(StrToIntDef(AnID, 0));
//lwm20061118  if ItemType = CH_SUM then UnlockDocument(StrToIntDef(AnID, 0));
//lwm20061118  if ItemType = CH_SUR then UnlockDocument(StrToIntDef(AnID, 0));
end;
...
//lwm20061118    LoadUnsignedOrders(OtherOrders, HaveOrders);
    ...
    begin
      AnID := OtherOrders[i];
//lwm20061118      Add(CH_ORD, AnID, TextForOrder(AnID), 'Other Unsigned', CanSign);
    end;
  finally
    StatusText('');
    HaveOrders.Free;
